...
class CheckDuplicate:
    ...
    def is_duplicate(img1_path, img2_path):
        ...
        img1_b64 = CheckDuplicate.encode_image(img1_path)
        img2_b64 = CheckDuplicate.encode_image(img2_path)
        ...
        message = HumanMessage(content=[
            {"type": "text", "text": DUPLICATE_PROMPT},
            {
                "type": "image_url",
                "image_url": {
                    "url": f"data:{CheckDuplicate.get_mime(img1_path)};base64,{img1_b64}"
                }
            },
            {
                "type": "image_url",
                "image_url": {
                    "url": f"data:{CheckDuplicate.get_mime(img2_path)};base64,{img2_b64}"
                }
            }
        ])
